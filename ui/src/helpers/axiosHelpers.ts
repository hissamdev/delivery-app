...
export async function getReq(route: string) {
    try {
        const res = await axios.get(`${SERVER_URL}${route && route}`);
        return res.data;
    } catch (error) {
...
export async function postReq(route: string, body: any) {
    try {
        console.log(`${SERVER_URL}${route && route}`, body);
        const res = await axios.post(`${SERVER_URL}${route && route}`, body);
        return res.data;
    } catch (error) {
        if (axios.isAxiosError(error)) {
            if (error.response) {
